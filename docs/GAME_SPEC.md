...
- [x] Fix the car falling **through** the city ground (tunnel-proof near-field collider — `HeightMapShape3D` / solid plateau box), make all static colliders tunnel-proof at the speed clamp, add a **recover / flip-upright** key + a **near-ground hover / landing assist**. *(Shipped session B, commit `9fd1ab0`: the zero-thickness terrain trimesh → a solid per-tile `HeightMapShape3D` sampled from the same `WorldData.HeightAt` the visual mesh uses (uniform scale + inverse-pre-scaled heights — the backend-safe recipe), un-tunnellable from either side; a **recover key (G)** that flattens pitch/roll keeping heading + zeroes spin authoritatively in `_IntegrateForces`; a **near-ground hover/landing assist** (down-ray + descent-damp force — anti-slam/anti-tunnel — with the optional up-spring off by default since it repels under `GravityScale=0`). Verified by session A: terrain-collider math is exact — grid + vertical (`Y≈66.5` plateau) + horizontal alignment, and `map_data[d·W+w]` matches the mesh fill order (no X/Z transpose); recover basis is right-handed & heading-preserving; hover is a framerate-independent force that rests at zero descent (the car sits on the now-solid ground); the contact-impulse→damage path and no-game-over are untouched. Scope is exactly 4 files — `world_main.res` byte-identical (no re-bake); shaders/palette/bake/`Traffic`/`MeshRegistry`/damage-model untouched; build 0/0; headless boot constructs the spawn-tile heightfield collider and exits 0. `[flight]` hover knobs (`hover_height=12`, `hover_strength=0`, `hover_damp=40`) in `settings.cfg`. Hands-on land/bounce/recover feel is the standing playtest gate.)*
- **Goal:** descend onto a street and bounce (slow drift **and** fast dive), recover frees a wedged car, you can settle + land; condition drops, the game never ends. → **brief:** [`TASK10-collision-solidity-and-comfort.md`](tasks/TASK10-collision-solidity-and-comfort.md) — **the last foundation task**. *(Playtest diagnostic: tunnelling is asymmetric/speed-dependent — fast dive passes through, slow climb is solid; root cause = the zero-thickness terrain trimesh. Fix = `HeightMapShape3D` + a near-ground descent cap.)*
